...
const buttonSizes: Record<ButtonSize, string> = {
  ...
  md: "w-[60px] flex items-center justify-center text-base rounded-full h-[60px]",
  lg: "px-8 py-4 text-lg rounded-full min-h-[60px]",
};

const ButtonSocials: React.FC<ButtonProps> = ({
  children,
  variant = "primary",
  size = "md",
  loading = false,
  ...
  const ButtonContent = () => (
    <div className="flex items-center gap-3">
      ...
      <span className="whitespace-nowrap font-heading text-xl pt-1">
        {children}
      </span>
    </div>
  );
